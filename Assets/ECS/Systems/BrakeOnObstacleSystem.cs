...
public class BreakOnObstacleSystem : ComponentSystem
{
	...
	protected override void OnUpdate()
	{
		...
		{
			var entity = NotBrakingCars.Entities[carIndex];
			...
			{
				if (Obstacles.Entities[obstacleIndex] == entity) //ignoruje samego siebie
				{
					continue;
				}
				var shouldBrake = distance(Obstacles.Obstacles[obstacleIndex].Position, NotBrakingCars.Positions[carIndex]) <= brakingDistance;
				if (shouldBrake)
				{
					if (EntityManager.HasComponent<Accelerating>(entity))
					{
						PostUpdateCommands.RemoveComponent<Accelerating>(entity);
					}
					PostUpdateCommands.AddComponent(entity, new Decelerating());
				}
			}
		}
	}
